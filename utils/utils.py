        ...
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():
                ...
                if test_save_path is not None:
                    ...
                    fig_gt = overlay_masks(image[ind, :, :], masks, labels=mask_labels, colors=cmap, alpha=0.5)
                    fig_pred = overlay_masks(image[ind, :, :], preds_o, labels=mask_labels, colors=cmap, alpha=0.5)
                    # Do with that image whatever you want to do.
                    # fig_gt.savefig(test_save_path + '/'+case + '_' +str(ind)+'_gt.png', bbox_inches="tight", dpi=300)
                    imageio.imwrite(test_save_path + '/' + case + '_' + str(ind) + '_gt.png', fig_gt, dpi=(300, 300))
                    # fig_pred.savefig(test_save_path + '/'+case + '_' +str(ind)+'_pred.png', bbox_inches="tight", dpi=300)
                    imageio.imwrite(test_save_path + '/' + case + '_' + str(ind) + '_pred.png', fig_pred, dpi=(300, 300))
                    plt.close('all')
